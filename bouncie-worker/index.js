/**
 * Bouncie GPS Proxy — Cloudflare Worker (YAV)
 *
 * Separate deployment from YKF's bouncie-proxy, same Bouncie account/app
 * (client_id: skycare-yk) but its own Worker + KV namespace, scoped to only
 * YAV's vehicle(s) — keeps each base's GPS data isolated the same way each
 * base has its own Firebase project.
 *
 * Secrets (Worker Settings > Variables & Secrets):
 *   BOUNCIE_CLIENT_ID     = skycare-yk
 *   BOUNCIE_CLIENT_SECRET = (same client secret as YKF's Worker — from bouncie.dev)
 *
 * KV binding (Worker Settings > Bindings > KV Namespace):
 *   Variable name: BOUNCIE_KV  →  Namespace: BOUNCIE_TOKENS_YAV
...
async function getAccessToken(env) {
  ...
  if (!refreshToken) throw new Error('No refresh_token in KV — add key "refresh_token" to BOUNCIE_TOKENS_YAV namespace');
